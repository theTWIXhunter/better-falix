...
if (!chrome || !chrome.storage || !chrome.storage.sync) {
  console.log('[better-falix] editor-wrapper-height: Extension context invalidated, stopping script');
  return;
}

chrome.storage.sync.get({ editorWrapperHeight: false, editorWrapperHeight_value: 600, enabled: true }, (data) => {
  if (chrome.runtime.lastError) {
    console.log('[better-falix] editor-wrapper-height: Chrome runtime error during initial load:', chrome.runtime.lastError);
    return;
  }
  ...
  function setEditorHeight() {
    ...
    try {
      chrome.storage.sync.get({ editorWrapperHeight: false, editorWrapperHeight_value: 600, enabled: true }, (storageData) => {
        ...
        if (!storageData.enabled || !storageData.editorWrapperHeight) {
        ...
        const height = parseInt(storageData.editorWrapperHeight_value, 10) || 600;
        document.querySelectorAll('.editor-wrapper').forEach(el => {
          // Only set height if not in fullscreen mode
