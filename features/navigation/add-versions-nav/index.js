...
chrome.storage.sync.get({ addVersionsNav: false, enabled: true }, (data) => {
  ...
  function addVersionsNavItem() {
    ...
    // Create the Versions nav item wrapped in li
    ...
    listItem.appendChild(versionsNavItem);

    // Find the collapsible content area (minecraftSection)
    const minecraftSection = document.getElementById('minecraftSection');
    if (minecraftSection) {
      // Find the navbar-nav ul inside
      const navbarNav = minecraftSection.querySelector('ul.navbar-nav');
      
      if (navbarNav) {
        // Add it as the first item in the list
        navbarNav.insertBefore(listItem, navbarNav.firstChild);
        console.log('[better-falix] add-versions-nav: Versions nav item added successfully');
      } else {
        console.log('[better-falix] add-versions-nav: navbar-nav not found');
      }
    } else {
